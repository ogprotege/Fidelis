...
- **v1.18.0 — the memory of the just** — two catechetical layers keyed by calendar date, sourced from the public domain and never AI-paraphrased (§13): **Saint of the Day** (the memorial name on the Today/Mass card links — purple acts — to a full Saint page: life, patronage, canonization, prayer, footnoted sources; deep-link `/saint/:day/:id`; reconciled to the engine's celebration by `saintForCelebration` token-match, since `Celebration` carries no id) and **Today in Church History** (the **sixth** Today card, under Mass before VOTD — the one deliberate raise of standing rule 2 from five to six, changed in the open across the harness guard, CLAUDE.md, Home.tsx, and the design spec; four honest states incl. a calm "no entry yet"; `/history/:day` lists a date's events oldest-first). New pipeline mirrors quotes: `scripts/build-{saints,history}.mjs` ← `scripts/*.corpus.json` → sealed `public/data/{saints,history}/<MM-DD>.json`, **PD-source-or-fail** provenance gate (the §3.3 analog), `verified` §3.4 ledger; lazy retry-after-rejection `loadSaints`/`loadHistory`. Ships a small `verified:false` seed (Butler's, 1913 Catholic Encyclopedia) that grows with no code change. Harness §33 (real `dayKey`/`saintForCelebration` logic + corpus integrity + shape guards); 14 e2e browser checks at a pinned date; no engine/lectionary/golden/sw changes; shells 1.18.0/11800. → [detail](docs/history/RELEASES.md#the-memory-of-the-just-v1180)
